...
const getPatientInfoByCIN = expressHandler(async (req, res) => {
  try {
      // Recherche du patient par CIN dans la base de données
      const patient = await Patient.findOne({ cin: req.params.cin });

      // Si le patient est trouvé, renvoyer ses informations
      if (patient) {
          res.status(200).json(patient);
      } else {
          // Si aucun patient correspondant au CIN n'est trouvé, renvoyer un message d'erreur
          res.status(404).json({ error: 'Aucun patient trouvé avec ce CIN.' });
      }
  } catch (error) {
      // Si une erreur se produit lors de la recherche du patient, renvoyer un message d'erreur
      console.error(error);
      res.status(500).json({ error: 'Erreur interne du serveur' });
  }
});

module.exports = { getPatient, addPatient, updatePatient, deletePatient, gePatientById,StatistiquePatient,globalPatient, getPatientInfoByCIN }; 
